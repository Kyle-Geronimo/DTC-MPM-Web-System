...
function updateUserById($conn, $userId) {
    ...
    // Update username and also update full_name so the name column reflects the change
    $stmt = $conn->prepare("UPDATE users SET username = ?, full_name = ?, updated_at = NOW() WHERE id = ?");
    if (!$stmt) return array('success' => false, 'message' => 'DB prepare error: ' . $conn->error);
    // use the username as the full_name display value
    $stmt->bind_param('ssi', $username, $username, $userId);
    if ($stmt->execute()) {
        $stmt->close();
        // Log activity: admin changed username
        $adminId = isset($_SESSION['user_id']) ? intval($_SESSION['user_id']) : null;
        $act = $conn->prepare("INSERT INTO activity_log (user_id, action, entity_type, entity_id, description) VALUES (?, 'update', 'user', ?, 'Username updated by admin')");
        if ($act) {
            $act->bind_param('ii', $adminId, $userId);
            $act->execute();
            $act->close();
        }
