...
// Note: allow all file types server-side (validate by size only).
// If you want to re-enable whitelisting later, restore $allowedMime.
...
// Server-side MIME detection is optional. We rely on file-size limit only.
...
if ($folderIdRaw === '') {
    // Upload to root (no folder). Use NULL for folder_id in DB.
    $folderId = null;
} else {
...
// ── Persist metadata ───────────────────────────────────────────
// Insert metadata. If $folderId is null, insert a NULL folder_id value.
if ($folderId === null) {
    ...
    if (!$stmt) { @unlink($destPath); respond_error('Database prepare failed. ' . $conn->error, ERR_DATABASE, 500); }
    $stmt->bind_param(
        'issssisis',
        $userId,
        $file['name'],
