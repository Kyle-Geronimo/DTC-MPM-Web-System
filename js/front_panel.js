...
setupModal(
    ...
    async (form, closeModal) => {
        ...
        const refreshAfter = async () => {
            try {
                const jobs = [];
                if (typeof loadTasks === 'function') jobs.push(loadTasks());
                if (typeof loadFrontPanelTaskSummary === 'function') jobs.push(loadFrontPanelTaskSummary());
                if (typeof loadDashboardStats === 'function') jobs.push(loadDashboardStats());
                await Promise.all(jobs);
            } catch (err) {
                console.error('Error refreshing after task save', err);
            }
        };
