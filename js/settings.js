...
async function loadSettings(){
    ...
    if (saved.theme) {
        if (typeof window.syncThemeButtons === 'function') {
            window.syncThemeButtons(saved.theme);
        } else {
            const t = saved.theme.toLowerCase();
            document.querySelectorAll('.theme-btn').forEach(b=>{
                if ((b.getAttribute('data-theme') || b.textContent.trim()).toLowerCase() === t) b.classList.add('active');
...
function renderApiKeys(){
    ...
    if (!keys.length){
        listEl.innerHTML = `<div class="settings-empty-state">
            <span class="empty-icon">🔑</span>
            <span class="empty-title">No API Keys</span>
            <span class="empty-desc">Generate an API key to connect external services.</span>
        </div>`;
        return;
    }
