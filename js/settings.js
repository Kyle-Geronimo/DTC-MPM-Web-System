...
async function initSettings() {
    ...
    document.querySelectorAll('.theme-btn').forEach(btn => btn.addEventListener('click', e => {
        ...
        applyTheme(theme);
        saveSettingsToLocal();
        // Reload so every page picks up the new theme immediately
        setTimeout(() => location.reload(), 150);
    }));

    // Timezone select
    const tzSelect = document.querySelector('#system .form-select');
    if (tzSelect) tzSelect.addEventListener('change', saveSettingsToLocal);
...
async function loadSettings(){
    ...
    // timezone
    if (saved.timezone){
        const tzEl = document.querySelector('#system select.form-select');
        if (tzEl) tzEl.value = saved.timezone;
    }
}
...
function saveSettingsToLocal(){
    ...
    // timezone
    const tz = document.querySelector('#system select.form-select');
    if (tz) saved.timezone = tz.value;
    localStorage.setItem('userSettings', JSON.stringify(saved));
}
