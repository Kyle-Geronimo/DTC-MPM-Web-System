...
function showFormErrors(form, messages) {
    ...
    if (!box) {
        ...
        box.style.cssText = 'background:#ffecec;border:1px solid #f5c6cb;color:#a33;padding:10px;margin-bottom:12px;border-radius:6px;';
...
async function loadDashboardStats() {
    try {
        ...
        if (result.success && result.data) {
            ...
            document.getElementById('activeProjectsCount').textContent = data.active_projects || 0;
            document.getElementById('projectsThisWeek').textContent = `+${data.projects_this_week || 0} this week`;
            
            // Update Completed Tasks
            document.getElementById('completedTasksCount').textContent = data.completed_tasks || 0;
            document.getElementById('tasksToday').textContent = `+${data.tasks_today || 0} today`;
            
            // Update Team Members
            document.getElementById('teamMembersCount').textContent = data.team_members || 0;
            document.getElementById('usersOnline').textContent = `${data.users_online || 0} online`;
            
            // Update Open Issues
            document.getElementById('openIssuesCount').textContent = data.open_issues || 0;
            document.getElementById('criticalIssues').textContent = `${data.critical_issues || 0} critical`;
