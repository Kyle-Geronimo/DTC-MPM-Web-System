...
function initializeQuickThemeSwitcher() {
    const headerRight = document.querySelector('.header-right');
    if (!headerRight || document.getElementById('quickThemeSwitcher')) return;

    const switcher = document.createElement('div');
    switcher.id = 'quickThemeSwitcher';
    switcher.className = 'quick-theme-switcher';
    ...
    switcher.innerHTML = `
        ...
        <button type="button" class="theme-btn" data-theme="auto" aria-pressed="false">Auto</button>
    `;

    const userMenuWrapper = headerRight.querySelector('#userMenuWrapper');
    if (userMenuWrapper) {
        headerRight.insertBefore(switcher, userMenuWrapper);
    } else {
    ...
    try {
        ...
        if (saved && saved.theme) {
            currentTheme = String(saved.theme).toLowerCase();
        } else if (document.body.classList.contains('dark')) {
            currentTheme = 'dark';
        }
    } catch (error) {
        if (document.body.classList.contains('dark')) currentTheme = 'dark';
    }

    window.syncThemeButtons(currentTheme);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add shared theme controls in the header for faster access
    initializeQuickThemeSwitcher();
