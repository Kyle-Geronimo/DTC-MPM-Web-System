                            ...
                            if (removeBtn) {
                                removeBtn.addEventListener('click', async function(e) {
                                    ...
                                    try {
                                        ...
                                        const res = await fetch('../settings/manage_group_members.php', { method: 'POST', body: fd });
                                        const data = await res.json();
                                        if (data && data.success) {
                                            LoadingState.success(this, 'Removed');
                                            // remove element from DOM
                                            memberEl.style.opacity = '0';
                                            memberEl.style.transform = 'scale(0.98)';
                                            setTimeout(() => memberEl.remove(), 250);
                                            // refresh stats
                                            loadTeamStats();
                                        } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // currentUser may not be loaded yet, so retry briefly
    if (window.currentUser) {
        restrictAdminButtons();
    } else {
        setTimeout(restrictAdminButtons, 500);
        setTimeout(restrictAdminButtons, 1500);
    }
});
