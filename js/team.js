...
function handleMemberButtons() {
    ...
    memberButtons.forEach(button => {
        button.addEventListener('click', async function() {
            ...
            // Helper: fetch and render members (so we can refresh after add/remove)
            const fetchAndRenderMembers = async () => {
                try {
                    ...
                    // Determine if current user can manage this team's members
                    const cu = window.currentUser || {};
                    const cuId = String(window.currentUserId || cu.id || '');
                    const isAdmin = (cu.role === 'admin');
                    const teamLeadId = teamCard.getAttribute('data-team-lead') || '';
                    const isLead = cuId && teamLeadId === cuId;
                        ...
                        members.forEach(member => {
                            ...
                            memberEl.innerHTML = `
                                ...
                                ${canManage ? `<div class="member-actions">
                                    <button class="btn-small btn-remove-member" data-user-id="${member.id}">Remove</button>
                                </div>` : ''}
                            `;
                            membersList.appendChild(memberEl);

                            // Bind remove handler for this member
                            const removeBtn = memberEl.querySelector('.btn-remove-member');
                            if (removeBtn) {
                                removeBtn.addEventListener('click', async function(e) {
                                    e.stopPropagation();
                                    if (!confirm('Remove ' + (member.full_name || member.username) + ' from this team?')) return;
                                    LoadingState.start(this, 'Removing...');
                                    try {
                                        const fd = new FormData();
                                        fd.append('user_id', member.id);
                                        fd.append('group_id', teamId);

                                        const res = await fetch('../settings/remove_member.php', { method: 'POST', body: fd });
                                        const data = await res.json();
                                        if (data && data.success) {
                                            LoadingState.success(this, 'Removed');
                                            // refresh list
                                            await fetchAndRenderMembers();
                                            loadTeamStats();
                                        } else {
                                            LoadingState.error(this, data.message || 'Failed');
                    ...
                    // If user can manage, add an "Add Member" button below the list
                    // Remove any existing add button first
                    const existingAdd = modal.querySelector('.btn-add-member');
                    if (existingAdd) existingAdd.remove();
                    if (canManage) {
                        const addWrapper = document.createElement('div');
                        addWrapper.className = 'member-add-wrapper';
                        addWrapper.style.marginTop = '12px';
                        addWrapper.innerHTML = `<button class="btn-primary btn-add-member">+ Add Member</button>`;
                        modal.querySelector('.modal-body').appendChild(addWrapper);

                        const addBtn = addWrapper.querySelector('.btn-add-member');
                        addBtn.addEventListener('click', async (ev) => {
                            ev.stopPropagation();
                            const fullName = (prompt('Enter full name of the member (First Last):') || '').trim();
                            if (!fullName) { Toast.error('Name required'); return; }
                            const email = (prompt('Enter email address of the member:') || '').trim();
                            ...
                            LoadingState.start(addBtn, 'Adding...');
                            try {
                                const fd = new FormData();
                                ...
                                const res = await fetch('../settings/add_member.php', { method: 'POST', body: fd });
                                const result = await res.json();
                                if (result && result.success) {
                                    LoadingState.success(addBtn, 'Added');
                                    Toast.success('Member added');
                                    // refresh members list
                                    await fetchAndRenderMembers();
                                    loadTeamStats();
                                } else {
                                    LoadingState.error(addBtn, 'Failed');
                                    Toast.error(result.message || 'Failed to add member');
                                    console.error('Add member failed', result);
                                }
                            } catch (err) {
                                LoadingState.error(addBtn, 'Error');
                    ...
                    LoadingState.error(this, 'Error');
                    console.error('Error loading team members:', err.message || err);
                }
            };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Hide admin-only buttons (Create Team) for non-admin users
    function restrictAdminButtons() {
        const cu = window.currentUser;
        const createBtn = document.getElementById('openCreateTeam');
        if (!createBtn) return;
        // Show only for admins; hide otherwise
        if (cu && cu.role === 'admin') {
    ...
    // currentUser may not be loaded yet, so try immediately and shortly after
    restrictAdminButtons();
    setTimeout(restrictAdminButtons, 300);
    setTimeout(restrictAdminButtons, 1200);
});
    ...
    try {
        ...
        teams.forEach(team => {
            try {
                ...
                el.setAttribute('data-team-lead', team.team_lead || '');
                
                // Check if current user is admin or team lead
                const cu = window.currentUser || {};
                const cuId = String(window.currentUserId || cu.id || '');
                const isAdmin = (cu.role === 'admin');
                const isLead = cuId && String(team.team_lead) === cuId;
                const canManage = isAdmin || isLead;
                ...
                el.innerHTML = `
                    ...
                    <div class="team-actions">
                        ...
                        ${canManage ? `<div class="team-actions-icons">
