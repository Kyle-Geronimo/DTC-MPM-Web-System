...
class ChatSystem {
    constructor() {
        ...
        this.isCreatingNewChat = false; // New state for creating chat mode
    ...
    renderNewChatInterface() {
        ...
        // Render the "To:" input interface with user list
        groupList.innerHTML = `
            <div class="new-chat-interface">
                <div class="new-chat-search-box">
                    <label for="newChatInput" style="display:block; font-size:12px; color:#666; margin-bottom:4px;">To:</label>
                    <input 
                        type="text" 
                        id="newChatInput" 
                        class="new-chat-input-field" 
        ...
        const inputField = document.getElementById('newChatInput');
        const userList = document.getElementById('newChatUserList');
        
        if (inputField) {
            inputField.addEventListener('input', (e) => {
                const query = e.target.value.toLowerCase();
                userList.innerHTML = this.renderFilteredUserList(query);
                
                // Attach click listeners to new user items
                this.attachNewChatUserListeners();
            });
    ...
    renderFilteredUserList(query) {
        ...
        if (filtered.length === 0) {
            if (query === '') {
                return `
                    <div class="new-chat-user-item" style="color: #999; text-align: center; padding: 20px; cursor: default;">
        ...
        return filtered.map(user => `
            <div class="new-chat-user-item" data-user-id="${user.id}" data-user-name="${this.escapeHtml(user.full_name || user.username)}">
                <div class="new-chat-user-avatar">${(user.full_name || user.username || '?').charAt(0).toUpperCase()}</div>
                <div class="new-chat-user-info">
                    <div class="new-chat-user-name">${this.escapeHtml(user.full_name || user.username)}</div>
                    <div class="new-chat-user-email">${this.escapeHtml(user.email)}</div>
                </div>
            </div>
        `).join('');
    }

    attachNewChatUserListeners() {
        const userItems = document.querySelectorAll('.new-chat-user-item[data-user-id]');
        userItems.forEach(item => {
            item.addEventListener('click', async (e) => {
                ...
                try {
                    ...
                    const user = this.allUsers.find(u => parseInt(u.id) === userId);
                    if (!user) throw new Error('User not found');

                    // Exit creating new chat mode
                    this.isCreatingNewChat = false;

                    // Enter direct-message mode
                    this.currentGroupId = null;
                    this.currentDirectUserId = user.id;
                    ...
                    await this.loadDirectMessages(user.id);

                    // Start auto-refresh for direct messages
                    if (this.messageRefreshInterval) clearInterval(this.messageRefreshInterval);
                    this.messageRefreshInterval = setInterval(() => this.loadDirectMessages(user.id), 5000);
                    ...
                    try { this.addMiniBubble(user); } catch (e) { console.error(e); }
                } catch (err) {
    ...
    attachEventListeners() {
        ...
        if (chatBackBtn) {
            chatBackBtn.addEventListener('click', (e) => {
                ...
                // If in creating new chat mode, exit it
        ...
        // Add button to start creating a new private/direct message conversation
        ...
        if (chatAddBtn) {
            chatAddBtn.addEventListener('click', async (e) => {
                ...
                // Show the "To:" input interface
                this.renderNewChatInterface();
                this.focusNewChatInput();
            });
        }
    }
