...
| Storage System | STORAGE_SYSTEM_IMPLEMENTATION.md | Doc/ |
| Emulator Environment | EMULATOR_ENVIRONMENT_SETUP.md | Doc/ |
...
### Features
- **File Upload:** Multipart upload with server-side MIME validation
- **Metadata in MySQL:** All file info stored in `storage_files` table
- **Secure Downloads:** Served through authenticated PHP handler
- **File Organisation:** Date-partitioned upload directories
- **phpMyAdmin Management:** Browse, filter, and export file records
- **Documentation:** [STORAGE_SYSTEM_IMPLEMENTATION.md](STORAGE_SYSTEM_IMPLEMENTATION.md)

---

## 🖥️ Emulator Environment (NEW)

### Supported Runtimes
| Runtime | Technology | Source stored as |
|---------|-----------|-----------------|
| Python | Pyodide (WebAssembly) | `source_code` text in MySQL |
| JavaScript | Sandboxed `Function()` | `source_code` text in MySQL |
| x86 Disk Image | v86 WebAssembly emulator | `binary_data` BLOB in MySQL |

### Features
- Programs stored in `emulator_programs` MySQL table
- In-browser execution — no server-side code execution
- Run-count tracking per program
- phpMyAdmin SQL queries for usage monitoring
- **Documentation:** [EMULATOR_ENVIRONMENT_SETUP.md](EMULATOR_ENVIRONMENT_SETUP.md)
